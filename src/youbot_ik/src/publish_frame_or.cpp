...
int main(int argc, char** argv)
{
  ...
  if(Beta<-2.5)//Pose might be inverted, to prevent that
    Beta=-3.14-Beta;
  ros::Duration(2).sleep();
  cout<<" Object goal wrt J2:"<<T_obj_J2(0,3)+.10<<endl;
  move_manip_js(time_m, step_m, rho3, T_obj_J2(0,3)+.10, Beta, rho2, rad(rho1), -Theta_5);//move arm to goal in desired time give data in m //.1 added to compensate for height of wheel kept below
  close_gripper();
  ros::Duration(2).sleep();
  transform_frame_3();
  while(ros::ok())
